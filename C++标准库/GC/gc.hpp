#ifndef RECYCLING_GC_HPP
#define RECYCLING_GC_HPP
...
namespace Recycling {
	namespace gc {
		...
		public:
			...
			template<class T, class = typename std::enable_if<std::is_convertible<T *, Class *>::value, void>::type>
			let(ref<T> rhs) {
				*this = rhs.lock();
			}

			let(Class* obj, referid id) : ptr(obj), id(id) {
			...
			template<typename T, class = typename std::enable_if<std::is_convertible<T *, Class *>::value, void>::type>
			operator ref<T>() {
				return ref<T>(id);
			}
