...
typedef struct file {
	char file_name[20];
	bool file_protect[3];
	bool open_file_protect[3]; // 仅在文件打开的时候有效
...
Map *queryUser(char userName[], MFD *mfd) {
	...
	m = mfd->head;
	while (m) {
		if (strcmp(userName, m->userNmae) == 0) {
			return m;
		}
		m = m->next;
	}
...
bool deleteFile(Map *user, char file_name[], AFD *afd) {
	...
	while (file) {
		if (strcmp(file_name, file->file_name) == 0) {
			cout << "\"" << file_name << "\" is open, please close it !\n";
			return false;
		}
		file = file->next;
	}
	...
	if (prefile->next == NULL) {
		cout << "user " << user->userName << "has not the file \"" << "\"" << endl;
	}
	return false;
}
	...
	if (file) {
		...
		*xfile = *file;
		...
		if (xfile->file_protect[0] >= open_file_protect[0]) {
			xfile->file_protect[0] = open_file_protect[0];
		}
		else {
		...
		if (xfile->file_protect[1] >= open_file_protect[1]) {
			xfile->file_protect[1] = open_file_protect[1];
		}
		else {
		...
		if (xfile->file_protect[2] >= open_file_protect[2]) {
			xfile->file_protect[2] = open_file_protect[2];
		}
		else {
		...
		if (afd->head == NULL) {
			...
			cout << "The open file too many !" << endl;
			return false;		
		}
	}
	else {
		cout << "the " << file_name << "is not exist!" << endl;
		return false;
	}
}
		...
		if (user == NULL) {
			...
			for (; ;) {
				...
				if (strcmp(command, "create") == 0) {
					cout << "Please input file(file_name file_protect, file_length): ";
					...
					cout << "Please input file_name which you want to delete: ";
					cin >> file_name;
					deleteFile(user, file_name, afd);
					displayUserFile(user);
				}
				else if (strcmp(command, "open") == 0) {
					cout << "Please input file_name which you want to open: ";
					...
					cout << "Please input file_name which you want to close: ";
					cin >> file_name;
					closeFile(afd, file_name);
					displayOpenFile(afd, user);
				}
				else if (strcmp(command, "read") == 0) {
					cout << "Please input file_name which you want to read: ";
					cin >> file_name;
					readFile(afd, file_name);
					displayOpenFile(afd, user);
				}
				else if (strcmp(command, "write") == 0) {
					cout << "Please input file_name which you want to write: ";
					cin >> file_name;
